...
impl FromStr for Status {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            "done" => Ok(Status::Done),
            invalid_status => Err(ParsingError(format!(
                "Invalid status \"{}\"",
                invalid_status
            ))),
        }
...
impl FromStr for TicketTitle {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        TicketTitle::new(s.to_string()).map_err(|err| ParsingError(err.to_string()))
    }
}
...
impl FromStr for TicketDescription {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        TicketDescription::new(s.to_string()).map_err(|err| ParsingError(err.to_string()))
    }
}
...
pub fn handle_command(
    ...
    match command {
        Command::Create { description, title } => {
            ...
            let ticket_id = ticket_store.save(draft);
            println!("Ticket created and assigned id: {ticket_id}");
        }
        Command::Edit {
            id,
            title,
            ...
            if let Some(updated_ticket) = ticket_store.update(&id, patch) {
                println!(
                    "The following ticket has been updated:\n{:?}",
                    updated_ticket
                );
            } else {
                println!("There was no ticket associated to the ticket id {:?}", id);
            }
        }
